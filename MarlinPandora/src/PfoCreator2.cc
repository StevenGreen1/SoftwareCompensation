...
//    if (trackList.empty())
//        std::cout << "The track list is empty!!!" << std::endl;

    std::cout << "===== Entering the track loop ===== " << std::endl;
    ...
    for (pandora::TrackList::const_iterator tIter = trackList.begin(), tIterEnd = trackList.end(); tIter != tIterEnd; ++tIter)
    {
        ...
        {        
            ...
            std::cout << "totalTrackMomentumAtDca : " << totalTrackMomentumAtDca << std::endl;
            const pandora::TrackList &pParentTrackList = pPandoraTrack->GetParentTrackList();
            for (pandora::TrackList::const_iterator ptIter = pParentTrackList.begin(), ptIterEnd = pParentTrackList.end(); ptIter != ptIterEnd; ++ptIter)
            {
                const pandora::Track *const pParentPandoraTrack(*ptIter);
                std::cout << "CanFormPfo                : " << pParentPandoraTrack->CanFormPfo() << std::endl;;
                std::cout << "CanFormClusterlessPfo     : " << pParentPandoraTrack->CanFormClusterlessPfo() << std::endl;;
                std::cout << "IsAvailable               : " << pParentPandoraTrack->IsAvailable() << std::endl;
                std::cout << "Momentum                  : " << pParentPandoraTrack->GetTrackStateAtStart().GetMomentum().GetMagnitude() << std::endl;
                const pandora::CartesianVector positionOfParentStart(pParentPandoraTrack->GetTrackStateAtStart().GetPosition());
                std::cout << "PositionAtStartX          : " << positionOfParentStart.GetX() << std::endl;
                std::cout << "PositionAtStartY          : " << positionOfParentStart.GetY() << std::endl;
                std::cout << "PositionAtStartZ          : " << positionOfParentStart.GetZ() << std::endl;
                const pandora::CartesianVector positionOfParentEnd(pParentPandoraTrack->GetTrackStateAtEnd().GetPosition());
                std::cout << "PositionAtEndX            : " << positionOfParentEnd.GetX() << std::endl;
                std::cout << "PositionAtEndY            : " << positionOfParentEnd.GetY() << std::endl;
                std::cout << "PositionAtEndZ            : " << positionOfParentEnd.GetZ() << std::endl;
            }
        ...
        else
        {
            ...
            std::cout << "trackMomentumAtDca      : " << trackMomentumAtDca << std::endl;
            std::cout << "totalTrackMomentumAtDca : " << totalTrackMomentumAtDca << std::endl;
        }
    }
    ...
    if (hasSiblings)
    {
        if (totalTrackMomentumAtStart < std::numeric_limits<float>::epsilon())
        {
            ...
            std::cout <<  "Problem with track.  Track has siblings. " << std::endl;
            ...
            for (pandora::TrackList::const_iterator tIter = trackList.begin(), tIterEnd = trackList.end(); tIter != tIterEnd; ++tIter)
            {
                ...
                std::cout << "totalTrackMomentumAtStart : " << totalTrackMomentumAtStart << std::endl;
                std::cout << "CanFormPfo                : " << pPandoraTrack->CanFormPfo() << std::endl;;
                std::cout << "CanFormClusterlessPfo     : " << pPandoraTrack->CanFormClusterlessPfo() << std::endl;;
                std::cout << "IsAvailable               : " << pPandoraTrack->IsAvailable() << std::endl;
                std::cout << "Momentum                  : " << pPandoraTrack->GetTrackStateAtStart().GetMomentum().GetMagnitude() << std::endl;
                const pandora::CartesianVector position(pPandoraTrack->GetTrackStateAtStart().GetPosition());
                std::cout << "PositionAtStartX          : " << position.GetX() << std::endl;
                std::cout << "PositionAtStartY          : " << position.GetY() << std::endl;
                std::cout << "PositionAtStartZ          : " << position.GetZ() << std::endl;
                const pandora::CartesianVector positionEnd(pPandoraTrack->GetTrackStateAtEnd().GetPosition());
                std::cout << "PositionAtEndX            : " << positionEnd.GetX() << std::endl;
                std::cout << "PositionAtEndY            : " << positionEnd.GetY() << std::endl;
                std::cout << "PositionAtEndZ            : " << positionEnd.GetZ() << std::endl;
            }
    ...
    else
    {
        if (totalTrackMomentumAtDca < std::numeric_limits<float>::epsilon())
        {
            ...
            std::cout <<  "Problem with track.  Track has no siblings. " << std::endl;
